...
class ManageDB:
    ...
    def add_user(self, user_info: dict) -> bool:
        ...
        self._session.add(
            User(vk_id=user_info['vk_id'], name=user_info['name'], surname=user_info['surname'], age=user_info['age'], sex=user_info['sex'],
                 city=user_info['city']))
    ...
    def add_blacklist(self, user_id: int, bl_id: int) -> bool:
        ...
        Возвращает True если добавлено\n"""
        x_ret = self._session.query(BlackList).where(BlackList.user_id == user_id)
        for x in x_ret.all():
            if x.user_fav_id == bl_id:
                return False
